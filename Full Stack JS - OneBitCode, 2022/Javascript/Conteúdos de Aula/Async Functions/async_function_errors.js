...
async function adjustPosition(x, y, z) {
  if(z > 90) {
    return Promise.reject("Ângulo Z inválido para a arma.")  // assim que retornamos e tratamos um erro numa async function.
  }
...
function moveAndFire(x, y, z) {
  adjustPosition(x, y, z).then(coord => {
    ...
    console.log(error) // Precisamos também colocar um catch error aqui.
  })
}
